...
using namespace cv;
using namespace fpm;
// using float = numeric::float<8, 24>;
#define INPUT_SIZE 224
#define INPUT_CHANNELS 3
void normalize(cv::Mat &image, const float mean[3], const float std[3]) {
    // Ensure the image is in fixeding point format
...
int main(int argc, char** argv)
{
    ...
    fixed_8_24 input[INPUT_SIZE * INPUT_SIZE * INPUT_CHANNELS];
    ...
    for (int i = 0; i < resizedImage.rows; i++) {
        ...
        // Get a pointer to the current pixel in resizedImage
            Vec3b pixel = resizedImage.at<Vec3b>(i, j);
        
        // Flatten the 3-channel pixel and copy each value to input array
        input[(i * resizedImage.cols + j) * 3 + 0] = fixed_8_24(pixel[0]);  // Blue channel
        input[(i * resizedImage.cols + j) * 3 + 1] = fixed_8_24(pixel[1]);  // Green channel
        input[(i * resizedImage.cols + j) * 3 + 2] = fixed_8_24(pixel[2]);  // Red channel
        }
    }
    ...
    fixed_8_24 layer0[layer0_size * kernel_size * INPUT_CHANNELS] = {fixed_8_24(0)};

    fixed_8_24 layer1[55 * 3 * 96] = {fixed_8_24(0)};
    fixed_8_24 layer2[Layer[2].size * Layer[2].cols * Layer[2].channels] = {fixed_8_24(0)};
    fixed_8_24 layer3[27 * 3 * 256] = {fixed_8_24(0)};
    fixed_8_24 layer4[15 * 3 * 256] = {fixed_8_24(0)};
    fixed_8_24 layer5[15 * 3 * 384] = {fixed_8_24(0)};
    fixed_8_24 layer6[15 * 3 * 384] = {fixed_8_24(0)};
    fixed_8_24 layer7[15 * 3 * 256] = {fixed_8_24(0)};
    fixed_8_24 layer8[6 * 6 * 256] = {fixed_8_24(0)};
    fixed_8_24 layer9[4096] = {fixed_8_24(0)};
    fixed_8_24 layer10[4096] = {fixed_8_24(0)};
    fixed_8_24 output[2] = {fixed_8_24(0)};
        ...
        for (int i = 0; i < Layer[0].size + Layer[1].padding + Layer[2].padding + Layer[3].padding + Layer[4].padding + 100; i++)
        {
            ...
            for (short k = 0; k < Layer[0].channels; k++)
            {
                if ((i < Layer[0].size) && (j < INPUT_SIZE + Layer[0].padding))
                {
                    ...
                    {
                        if (j < Layer[0].cols)
                            layer0[(i * Layer[0].cols) + (j) + (k * layer0_size * Layer[0].cols)] = fixed_8_24(0);
                        else
                            layer0[(i * Layer[0].cols) + (Layer[0].cols - 1) + (k * layer0_size * Layer[0].cols)] = fixed_8_24(0);
                    }
                    else
                    {
